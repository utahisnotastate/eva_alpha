...
import {updateAppointmentForm, getAppointmentForm} from '../../../api/appointment.api.js'
...
export default function FormTemplate(props) {
    ...
    const appointmentformdetails = useSelector(state => state.appointment.activeappointmentform.activeAppointmentFormDetails);
    const appointmentformfields = useSelector(state => state.appointment.activeappointmentform.activeAppointmentFormFields);
    const masteractiveappointmentformfields = useSelector(state => state.appointment.activeappointmentform.masteractiveappointmentformfields)
    ...
    const onSubmit = data => {
       ...
       updateAppointmentForm(props.appointmentId, appointmentform).catch((error) =>console.log(error))
    };
    ...
    useEffect(() => {
        fetchForm(props.formId).then(response => {
            dispatch({type: 'set_master_active_appointment_form_fields', form: response.form})
        })
    }, [])
            ...
            <FormProvider {...methods}>
                <form onSubmit={methods.handleSubmit(onSubmit)}>
                    ...
                    {Object.keys(appointmentformfields).map(field => (
                        <div key={field}>
                            <AppointmentFormFieldContainer name={field} fieldprops={appointmentformfields[field]}/>
                        </div>
                    ))}

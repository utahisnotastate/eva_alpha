...
import MedicationAndDosageSearch from "../../../basestyledcomponents/MedicationAndDosageSearch/medicationanddosagesearch";
...
export default function AddNewMedicationForm(props) {
  ...
  const icd10assessmentcode = useSelector(
    (state) => state.patient.addmedicationformicd10result.icd10assessmentcode
  );
  const assessment_description = useSelector(
    (state) => state.patient.addmedicationformicd10result.assessment_description
  );
  const medicationname = useSelector(
    (state) => state.patient.addMedicationFormMedication
  );
  let { id } = useParams();
  const onSubmit = (data) => {
    ...
    axios
      .post(`${API_URL}/patients/${id}/medications/`, allDiagnosisData)
      .then((response) => {

	...
	useEffect(() => {
		...
		fetchForm(formId).then((response) => {
			...
			if (response.form.customformfields) {
				console.log(
					'response form is: ' + JSON.stringify(response.form)
				)
				dispatch({
					type: 'load_form_fields',
					newformfields: response.form.customformfields,
									...
									<Grid container justify={`flex-end`}>
										<Grid item>
											<Typography variant="h2">
												MAKE SURE FORM IS SAVED BEFORE
												PREVIEWING
											</Typography>
										</Grid>
										<Grid item>
								...
								<CardBody>
									<Grid container direction="column">
										...
										<Grid item>
											...
											<select
												className={
													classes.formTypeSelectContainer
												}
												name="form_type"

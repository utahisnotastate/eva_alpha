...
function SectionComponent(props) {
	...
	return (
		<Droppable droppableId="sectioncomponent">
			{(provided, snapshot) => (
				<div
					className={`section-tool-container`}
...
export default function FormsCenter() {
	const [formbuildertools, setFormBuilderTools] = useState([
		basictool,
		formtool,
	])
	...
	function onDragEnd(result) {
		...
		if (
			result.source.droppableId === 'formtools' &&
			result.destination.droppableId === 'formbuilder'
		) {
			console.log(
				formbuildertools.find(
					(formbuildertool) =>
						formbuildertool.id === result.draggableId
				)
			)
			let copiedformtool = formbuildertools.find(
				(formbuildertool) => formbuildertool.id === result.draggableId
			)
			...
			console.log({
				...
				name: copiedformtool.name,
				component: copiedformtool.component,
			})
			customformcomponents.push({
					...
					<GridItem xs={10}>
						<Card>
							...
							<CardBody>
								<GridContainer>
									<GridItem
										className={`formbuilder-droppable-container`}>
										<Droppable droppableId="formbuilder">
											{(provided, snapshot) => (
												<div ref={provided.innerRef}>
													{customformcomponents.map(
														(tool, index) => (
															<Draggable
																key={tool.id}
																draggableId={
